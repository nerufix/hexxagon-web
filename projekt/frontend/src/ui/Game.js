...
function Game({ id, user, game, client, ...props }) {
  ...
  useEffect(() => {
    client.subscribe('moves/'+id, { qos: 1 })
  }, [])
  ...
  const [inviteClicked, setInviteClicked] = useState([])
  ...
  const handleMoveClick = (hex) => {
    ...
    if (isOneTileApart(hex, atSelect)) {
      ...
      client.publish('moves/'+id, JSON.stringify(dataToSend), {qos: 1})
  ...
  const handleInviteClick = (player) => {
    ...
    client.publish('invite/'+player, JSON.stringify({id, from: user.login}), {qos: 1})
  }

  const getWaitingOptions = () => (
    <div className="d-flex flex-wrap justify-content-center">
      <div className="text-white h3"><Loading />Wait for another player to join...</div>
      <div className="break" />
      {props.lobbyPlayers.length>0 && 
      <div className="bg-dark text-white m-2 p-2">
        <div>Invite players:</div>
        {props.lobbyPlayers.map(el => (
          <div className="mt-2">
            {inviteClicked.includes(el) ? "Invite sent!" : (
              <>{el} <Button variant="outline-info" onClick={() => handleInviteClick(el)}>Invite</Button></>
            )}
          </div>
        ))}
      </div>}
  ...
  return !game.board ? (<Loading />) : (
    <div className="p-3">
      {
      game.players.length<2 ? getWaitingOptions() :
      (<>
...
const mapStateToProps = (state, props) => {
  return {
    ...
    lobbyPlayers: state.user.playerLocation.lobby
  }
}
